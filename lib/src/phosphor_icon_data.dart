library phosphor_flutter;

import 'package:flutter/widgets.dart';

typedef PhosphorIconData = IconData;
typedef PhosphorFlatIconData = IconData;
typedef PhosphorDuotoneIconData = IconData;

const String _fontPackage = 'phosphor_flutter';
const String _duotoneFontFamily = 'PhosphorDuotone';

/// Constructs a [PhosphorFlatIconData] for a runtime [codePoint] and [style].
///
/// [style] is case-insensitive: `'regular'`, `'Regular'`, `'bold'`, `'fill'`,
/// `'light'`, `'thin'` are all valid.
///
/// Use this instead of the removed `PhosphorFlatIconData(code, style)`
/// constructor when the codepoint is a runtime value (e.g. stored in the DB).
///
/// ```dart
/// phosphorIcon(label.iconCode, 'regular')
/// ```
PhosphorFlatIconData phosphorIcon(int codePoint, String style) {
  final s = style[0].toUpperCase() + style.substring(1).toLowerCase();
  return IconData(
    codePoint,
    fontFamily: 'Phosphor$s',
    fontPackage: _fontPackage,
    matchTextDirection: true,
  );
}

IconData? phosphorDuotoneSecondaryIcon(IconData? icon) {
  if (icon == null ||
      icon.fontPackage != _fontPackage ||
      icon.fontFamily != _duotoneFontFamily) {
    return null;
  }

  return _phosphorDuotoneSecondaryIcons[icon.codePoint];
}

bool isPhosphorDuotoneIcon(IconData? icon) =>
    phosphorDuotoneSecondaryIcon(icon) != null;

const Map<int, IconData> _phosphorDuotoneSecondaryIcons = <int, IconData>{
  0xeb9b: IconData(
    0xeb9a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6f9: IconData(
    0xe6f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee4f: IconData(
    0xee4e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecd9: IconData(
    0xecd8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe003: IconData(
    0xe002,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4ff: IconData(
    0xe4fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe503: IconData(
    0xe502,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe505: IconData(
    0xe504,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe501: IconData(
    0xe500,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5d7: IconData(
    0xe5d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe005: IconData(
    0xe004,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe007: IconData(
    0xe006,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8a7: IconData(
    0xe8a6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe507: IconData(
    0xe506,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb0d: IconData(
    0xeb0c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe50b: IconData(
    0xe50a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb0f: IconData(
    0xeb0e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe50d: IconData(
    0xe50c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb11: IconData(
    0xeb10,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe50f: IconData(
    0xe50e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaef: IconData(
    0xeaee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe511: IconData(
    0xe510,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb13: IconData(
    0xeb12,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe513: IconData(
    0xe512,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb15: IconData(
    0xeb14,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe96d: IconData(
    0xe96c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe573: IconData(
    0xe572,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe515: IconData(
    0xe514,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5d9: IconData(
    0xe5d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe009: IconData(
    0xe008,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7bd: IconData(
    0xe7bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb81: IconData(
    0xeb80,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe00b: IconData(
    0xe00a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe975: IconData(
    0xe974,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5db: IconData(
    0xe5da,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe517: IconData(
    0xe516,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb97: IconData(
    0xeb96,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedab: IconData(
    0xedaa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe00d: IconData(
    0xe00c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe013: IconData(
    0xe012,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe015: IconData(
    0xe014,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe017: IconData(
    0xe016,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe03b: IconData(
    0xe03a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe03d: IconData(
    0xe03c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe019: IconData(
    0xe018,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe01b: IconData(
    0xe01a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe01d: IconData(
    0xe01c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe01f: IconData(
    0xe01e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe021: IconData(
    0xe020,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe023: IconData(
    0xe022,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe025: IconData(
    0xe024,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe027: IconData(
    0xe026,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe029: IconData(
    0xe028,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe02b: IconData(
    0xe02a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe02d: IconData(
    0xe02c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe05b: IconData(
    0xe05a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe02f: IconData(
    0xe02e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe031: IconData(
    0xe030,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe033: IconData(
    0xe032,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe035: IconData(
    0xe034,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe037: IconData(
    0xe036,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe039: IconData(
    0xe038,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe03f: IconData(
    0xe03e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe041: IconData(
    0xe040,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe043: IconData(
    0xe042,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe045: IconData(
    0xe044,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe047: IconData(
    0xe046,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe049: IconData(
    0xe048,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe04b: IconData(
    0xe04a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe04d: IconData(
    0xe04c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe04f: IconData(
    0xe04e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe051: IconData(
    0xe050,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe053: IconData(
    0xe052,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe055: IconData(
    0xe054,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe057: IconData(
    0xe056,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe519: IconData(
    0xe518,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe51b: IconData(
    0xe51a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe51d: IconData(
    0xe51c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe51f: IconData(
    0xe51e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe521: IconData(
    0xe520,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe523: IconData(
    0xe522,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe525: IconData(
    0xe524,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe527: IconData(
    0xe526,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe529: IconData(
    0xe528,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe52b: IconData(
    0xe52a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe52d: IconData(
    0xe52c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe52f: IconData(
    0xe52e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe059: IconData(
    0xe058,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe05d: IconData(
    0xe05c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe05f: IconData(
    0xe05e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe061: IconData(
    0xe060,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe063: IconData(
    0xe062,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe065: IconData(
    0xe064,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe067: IconData(
    0xe066,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe069: IconData(
    0xe068,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe06b: IconData(
    0xe06a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe06d: IconData(
    0xe06c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe06f: IconData(
    0xe06e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe071: IconData(
    0xe070,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe073: IconData(
    0xe072,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5dd: IconData(
    0xe5dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe075: IconData(
    0xe074,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5df: IconData(
    0xe5de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe077: IconData(
    0xe076,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe079: IconData(
    0xe078,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe07b: IconData(
    0xe07a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe07d: IconData(
    0xe07c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe07f: IconData(
    0xe07e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe081: IconData(
    0xe080,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe083: IconData(
    0xe082,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe085: IconData(
    0xe084,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe087: IconData(
    0xe086,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe089: IconData(
    0xe088,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe08b: IconData(
    0xe08a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe08d: IconData(
    0xe08c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe08f: IconData(
    0xe08e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe091: IconData(
    0xe090,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe093: IconData(
    0xe092,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe095: IconData(
    0xe094,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe097: IconData(
    0xe096,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe099: IconData(
    0xe098,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb07: IconData(
    0xeb06,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe09b: IconData(
    0xe09a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe09d: IconData(
    0xe09c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe531: IconData(
    0xe530,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe533: IconData(
    0xe532,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe09f: IconData(
    0xe09e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0a1: IconData(
    0xe0a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed3f: IconData(
    0xed3e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0a3: IconData(
    0xe0a2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0a5: IconData(
    0xe0a4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe535: IconData(
    0xe534,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe537: IconData(
    0xe536,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0a7: IconData(
    0xe0a6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed3d: IconData(
    0xed3c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb05: IconData(
    0xeb04,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0a9: IconData(
    0xe0a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5e1: IconData(
    0xe5e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5e3: IconData(
    0xe5e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee35: IconData(
    0xee34,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0ab: IconData(
    0xe0aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe833: IconData(
    0xe832,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0ad: IconData(
    0xe0ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5e5: IconData(
    0xe5e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee05: IconData(
    0xee04,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9fd: IconData(
    0xe9fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe775: IconData(
    0xe774,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe819: IconData(
    0xe818,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe923: IconData(
    0xe922,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0af: IconData(
    0xe0ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0b1: IconData(
    0xe0b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5e7: IconData(
    0xe5e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe76d: IconData(
    0xe76c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0b3: IconData(
    0xe0b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0b5: IconData(
    0xe0b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0b7: IconData(
    0xe0b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0b9: IconData(
    0xe0b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec73: IconData(
    0xec72,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe949: IconData(
    0xe948,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe71b: IconData(
    0xe71a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea29: IconData(
    0xea28,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee4b: IconData(
    0xee4a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe965: IconData(
    0xe964,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe725: IconData(
    0xe724,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe81f: IconData(
    0xe81e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0bb: IconData(
    0xe0ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0bd: IconData(
    0xe0bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0bf: IconData(
    0xe0be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0c1: IconData(
    0xe0c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0c3: IconData(
    0xe0c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0c5: IconData(
    0xe0c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0c7: IconData(
    0xe0c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe809: IconData(
    0xe808,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec51: IconData(
    0xec50,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7c7: IconData(
    0xe7c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7c5: IconData(
    0xe7c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7c3: IconData(
    0xe7c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7bf: IconData(
    0xe7be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7c1: IconData(
    0xe7c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0c9: IconData(
    0xe0c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0cb: IconData(
    0xe0ca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed25: IconData(
    0xed24,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea2b: IconData(
    0xea2a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0cd: IconData(
    0xe0cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7b1: IconData(
    0xe7b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb63: IconData(
    0xeb62,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7f5: IconData(
    0xe7f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0cf: IconData(
    0xe0ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5e9: IconData(
    0xe5e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0d1: IconData(
    0xe0d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5eb: IconData(
    0xe5ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0d3: IconData(
    0xe0d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5ed: IconData(
    0xe5ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0d5: IconData(
    0xe0d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5ef: IconData(
    0xe5ee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea2d: IconData(
    0xea2c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb01: IconData(
    0xeb00,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0d7: IconData(
    0xe0d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee61: IconData(
    0xee60,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea65: IconData(
    0xea64,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9e1: IconData(
    0xe9e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe72d: IconData(
    0xe72c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeda1: IconData(
    0xeda0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0db: IconData(
    0xe0da,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0dd: IconData(
    0xe0dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0df: IconData(
    0xe0de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0e1: IconData(
    0xe0e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe787: IconData(
    0xe786,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee0b: IconData(
    0xee0a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7f3: IconData(
    0xe7f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0e3: IconData(
    0xe0e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0e5: IconData(
    0xe0e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0e7: IconData(
    0xe0e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8f3: IconData(
    0xe8f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xede1: IconData(
    0xede0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0e9: IconData(
    0xe0e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0eb: IconData(
    0xe0ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0ed: IconData(
    0xe0ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5f1: IconData(
    0xe5f0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe759: IconData(
    0xe758,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeccb: IconData(
    0xecca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe723: IconData(
    0xe722,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6cf: IconData(
    0xe6ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaa5: IconData(
    0xeaa4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8e5: IconData(
    0xe8e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea35: IconData(
    0xea34,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe00f: IconData(
    0xe00e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee55: IconData(
    0xee54,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea37: IconData(
    0xea36,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe863: IconData(
    0xe862,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe861: IconData(
    0xe860,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe865: IconData(
    0xe864,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe85f: IconData(
    0xe85e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe74f: IconData(
    0xe74e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6b5: IconData(
    0xe6b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe81d: IconData(
    0xe81c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea69: IconData(
    0xea68,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0ef: IconData(
    0xe0ee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5f3: IconData(
    0xe5f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0f3: IconData(
    0xe0f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec55: IconData(
    0xec54,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0f5: IconData(
    0xe0f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0f7: IconData(
    0xe0f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5f5: IconData(
    0xe5f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5f7: IconData(
    0xe5f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5f9: IconData(
    0xe5f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe101: IconData(
    0xe100,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe103: IconData(
    0xe0fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe104: IconData(
    0xe0ff,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe105: IconData(
    0xe102,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec6d: IconData(
    0xec6c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe107: IconData(
    0xe106,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea6f: IconData(
    0xea6e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe49d: IconData(
    0xe49c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe919: IconData(
    0xe918,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe781: IconData(
    0xe780,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe539: IconData(
    0xe538,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe109: IconData(
    0xe108,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe10b: IconData(
    0xe10a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe713: IconData(
    0xe712,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7b3: IconData(
    0xe7b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7b5: IconData(
    0xe7b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8b1: IconData(
    0xe8b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea15: IconData(
    0xea14,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe715: IconData(
    0xe714,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea13: IconData(
    0xea12,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8b3: IconData(
    0xe8b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe10d: IconData(
    0xe10c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7df: IconData(
    0xe7de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe10f: IconData(
    0xe10e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec59: IconData(
    0xec58,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7a5: IconData(
    0xe7a4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe111: IconData(
    0xe110,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9d9: IconData(
    0xe9d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe113: IconData(
    0xe112,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee31: IconData(
    0xee30,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8cd: IconData(
    0xe8cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe115: IconData(
    0xe114,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5fb: IconData(
    0xe5fa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe0f9: IconData(
    0xe0f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee51: IconData(
    0xee50,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe117: IconData(
    0xe116,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe119: IconData(
    0xe118,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe11b: IconData(
    0xe11a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe11d: IconData(
    0xe11c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe11f: IconData(
    0xe11e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe121: IconData(
    0xe120,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe123: IconData(
    0xe122,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe125: IconData(
    0xe124,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe13f: IconData(
    0xe13e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe127: IconData(
    0xe126,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe129: IconData(
    0xe128,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe12b: IconData(
    0xe12a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe12d: IconData(
    0xe12c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe137: IconData(
    0xe136,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe139: IconData(
    0xe138,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe135: IconData(
    0xe134,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe133: IconData(
    0xe132,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe131: IconData(
    0xe130,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe12f: IconData(
    0xe12e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe13b: IconData(
    0xe13a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe13d: IconData(
    0xe13c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe141: IconData(
    0xe140,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed39: IconData(
    0xed38,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed81: IconData(
    0xed80,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed2f: IconData(
    0xed2e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9d1: IconData(
    0xe9d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe749: IconData(
    0xe748,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe143: IconData(
    0xe142,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe145: IconData(
    0xe144,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe147: IconData(
    0xe146,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe149: IconData(
    0xe148,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe14d: IconData(
    0xe14c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe14f: IconData(
    0xe14e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebab: IconData(
    0xebaa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe767: IconData(
    0xe766,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe951: IconData(
    0xe950,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5fd: IconData(
    0xe5fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5ff: IconData(
    0xe5fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe601: IconData(
    0xe600,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeacb: IconData(
    0xeaca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8d1: IconData(
    0xe8d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe151: IconData(
    0xe150,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe153: IconData(
    0xe152,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaa7: IconData(
    0xeaa6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe155: IconData(
    0xe154,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8b7: IconData(
    0xe8b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe157: IconData(
    0xe156,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe159: IconData(
    0xe158,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe15b: IconData(
    0xe15a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaa9: IconData(
    0xeaa8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaad: IconData(
    0xeaac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe15d: IconData(
    0xe15c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe161: IconData(
    0xe160,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe165: IconData(
    0xe164,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe163: IconData(
    0xe162,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe167: IconData(
    0xe166,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe169: IconData(
    0xe168,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe16d: IconData(
    0xe16c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe16b: IconData(
    0xe16a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe16f: IconData(
    0xe16e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe171: IconData(
    0xe170,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe15f: IconData(
    0xe15e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe173: IconData(
    0xe172,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe177: IconData(
    0xe176,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe175: IconData(
    0xe174,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe179: IconData(
    0xe178,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe17b: IconData(
    0xe17a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe17d: IconData(
    0xe17c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe17f: IconData(
    0xe17e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe181: IconData(
    0xe180,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe183: IconData(
    0xe182,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe185: IconData(
    0xe184,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeba7: IconData(
    0xeba6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe187: IconData(
    0xe186,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe189: IconData(
    0xe188,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8c5: IconData(
    0xe8c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe53b: IconData(
    0xe53a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea4b: IconData(
    0xea4a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9ff: IconData(
    0xe9fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed8f: IconData(
    0xed8e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe831: IconData(
    0xe830,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeceb: IconData(
    0xecea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed91: IconData(
    0xed90,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed93: IconData(
    0xed92,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe18b: IconData(
    0xe18a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe603: IconData(
    0xe602,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe18d: IconData(
    0xe18c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe18f: IconData(
    0xe18e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb45: IconData(
    0xeb44,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe191: IconData(
    0xe190,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe193: IconData(
    0xe192,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe195: IconData(
    0xe194,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9c3: IconData(
    0xe9c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea6b: IconData(
    0xea6a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe197: IconData(
    0xe196,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe199: IconData(
    0xe198,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe19b: IconData(
    0xe19a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe19d: IconData(
    0xe19c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe19f: IconData(
    0xe19e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed2d: IconData(
    0xed2c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1a1: IconData(
    0xe1a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeded: IconData(
    0xedec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1a5: IconData(
    0xe1a4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1ab: IconData(
    0xe1aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1ad: IconData(
    0xe1ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1af: IconData(
    0xe1ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1b1: IconData(
    0xe1b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe53d: IconData(
    0xe53c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1b3: IconData(
    0xe1b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe53f: IconData(
    0xe53e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1b5: IconData(
    0xe1b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1b7: IconData(
    0xe1b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1b9: IconData(
    0xe1b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe541: IconData(
    0xe540,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea99: IconData(
    0xea98,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea97: IconData(
    0xea96,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedc9: IconData(
    0xedc8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1bb: IconData(
    0xe1ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7ff: IconData(
    0xe7fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7cf: IconData(
    0xe7ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1bd: IconData(
    0xe1bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaff: IconData(
    0xeafe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1bf: IconData(
    0xe1be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe979: IconData(
    0xe978,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea07: IconData(
    0xea06,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1c3: IconData(
    0xe1c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1c1: IconData(
    0xe1c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe60f: IconData(
    0xe60e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb49: IconData(
    0xeb48,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe78f: IconData(
    0xe78e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe547: IconData(
    0xe546,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe545: IconData(
    0xe544,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe543: IconData(
    0xe542,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1c5: IconData(
    0xe1c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1c9: IconData(
    0xe1c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1c7: IconData(
    0xe1c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea0f: IconData(
    0xea0e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe549: IconData(
    0xe548,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe81b: IconData(
    0xe81a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed43: IconData(
    0xed42,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeca7: IconData(
    0xeca6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6cb: IconData(
    0xe6ca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe765: IconData(
    0xe764,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1cb: IconData(
    0xe1ca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1cd: IconData(
    0xe1cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe86b: IconData(
    0xe86a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe54b: IconData(
    0xe54a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1cf: IconData(
    0xe1ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1d1: IconData(
    0xe1d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7f7: IconData(
    0xe7f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee37: IconData(
    0xee36,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeabf: IconData(
    0xeabe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed13: IconData(
    0xed12,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe611: IconData(
    0xe610,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed4b: IconData(
    0xed48,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed4d: IconData(
    0xed49,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1d3: IconData(
    0xe1d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee13: IconData(
    0xee12,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1d5: IconData(
    0xe1d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8a1: IconData(
    0xe8a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1d7: IconData(
    0xe1d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1d9: IconData(
    0xe1d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe615: IconData(
    0xe614,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee5f: IconData(
    0xee5e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe617: IconData(
    0xe616,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1db: IconData(
    0xe1da,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed0b: IconData(
    0xed0a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec7d: IconData(
    0xec7c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe619: IconData(
    0xe618,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe54d: IconData(
    0xe54c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe54f: IconData(
    0xe54e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe551: IconData(
    0xe550,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe553: IconData(
    0xe552,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeadb: IconData(
    0xeada,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe555: IconData(
    0xe554,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe557: IconData(
    0xe556,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe559: IconData(
    0xe558,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe55b: IconData(
    0xe55a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe55d: IconData(
    0xe55c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec4d: IconData(
    0xec4c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb53: IconData(
    0xeb52,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe55f: IconData(
    0xe55e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1dd: IconData(
    0xe1dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7c9: IconData(
    0xe7c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7d9: IconData(
    0xe7d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8fd: IconData(
    0xe8fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1df: IconData(
    0xe1de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed17: IconData(
    0xed16,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe561: IconData(
    0xe560,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe563: IconData(
    0xe562,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe83f: IconData(
    0xe83e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed0f: IconData(
    0xed0e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1e1: IconData(
    0xe1e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1e3: IconData(
    0xe1e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee47: IconData(
    0xee46,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1e5: IconData(
    0xe1e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedf3: IconData(
    0xedf2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1e7: IconData(
    0xe1e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1e9: IconData(
    0xe1e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1eb: IconData(
    0xe1ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeba5: IconData(
    0xeba4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1ed: IconData(
    0xe1ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8f5: IconData(
    0xe8f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1ef: IconData(
    0xe1ee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1f1: IconData(
    0xe1f0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1f3: IconData(
    0xe1f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1f5: IconData(
    0xe1f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1f7: IconData(
    0xe1f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1f9: IconData(
    0xe1f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe565: IconData(
    0xe564,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed99: IconData(
    0xed98,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe61b: IconData(
    0xe61a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1fb: IconData(
    0xe1fa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe925: IconData(
    0xe924,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe74b: IconData(
    0xe74a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe61d: IconData(
    0xe61c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7e7: IconData(
    0xe7e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecdf: IconData(
    0xecde,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xece1: IconData(
    0xece0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1fd: IconData(
    0xe1fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe795: IconData(
    0xe794,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeae3: IconData(
    0xeae2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1ff: IconData(
    0xe1fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe201: IconData(
    0xe200,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe203: IconData(
    0xe202,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe205: IconData(
    0xe204,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe207: IconData(
    0xe206,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe209: IconData(
    0xe208,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe20b: IconData(
    0xe20a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe20d: IconData(
    0xe20c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea7f: IconData(
    0xea7e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe94f: IconData(
    0xe94e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe20f: IconData(
    0xe20e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed75: IconData(
    0xed74,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe211: IconData(
    0xe210,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe567: IconData(
    0xe566,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb41: IconData(
    0xeb40,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee33: IconData(
    0xee32,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe955: IconData(
    0xe954,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7d1: IconData(
    0xe7d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe70d: IconData(
    0xe70c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe70f: IconData(
    0xe70e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe813: IconData(
    0xe812,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb65: IconData(
    0xeb64,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe213: IconData(
    0xe212,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6af: IconData(
    0xe6ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecc1: IconData(
    0xecc0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedbd: IconData(
    0xedbc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea81: IconData(
    0xea80,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe215: IconData(
    0xe214,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe217: IconData(
    0xe216,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe219: IconData(
    0xe218,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe21b: IconData(
    0xe21a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebbd: IconData(
    0xebbc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe21d: IconData(
    0xe21c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe21f: IconData(
    0xe21e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecbb: IconData(
    0xecba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecbd: IconData(
    0xecbc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe743: IconData(
    0xe742,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee45: IconData(
    0xee44,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe883: IconData(
    0xe882,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe881: IconData(
    0xe880,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaf1: IconData(
    0xeaf0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe221: IconData(
    0xe220,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe223: IconData(
    0xe222,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe225: IconData(
    0xe224,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe569: IconData(
    0xe568,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeac5: IconData(
    0xeac4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7bb: IconData(
    0xe7ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee5d: IconData(
    0xee5c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe56b: IconData(
    0xe56a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe227: IconData(
    0xe226,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe761: IconData(
    0xe760,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe229: IconData(
    0xe228,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe22b: IconData(
    0xe22a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9df: IconData(
    0xe9de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9f3: IconData(
    0xe9f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec71: IconData(
    0xec70,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6a7: IconData(
    0xe6a6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe22d: IconData(
    0xe22c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9c1: IconData(
    0xe9c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed67: IconData(
    0xed66,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe22f: IconData(
    0xe22e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe231: IconData(
    0xe230,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb2b: IconData(
    0xeb2a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe233: IconData(
    0xe232,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe61f: IconData(
    0xe61e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea21: IconData(
    0xea20,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb36: IconData(
    0xeb32,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb31: IconData(
    0xeb30,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe95f: IconData(
    0xe95e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe915: IconData(
    0xe914,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb2f: IconData(
    0xeb2e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb37: IconData(
    0xeb34,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb1d: IconData(
    0xeb1c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe705: IconData(
    0xe704,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb1f: IconData(
    0xeb1e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb39: IconData(
    0xeb38,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea25: IconData(
    0xea24,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb3b: IconData(
    0xeb33,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb1b: IconData(
    0xeb1a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb25: IconData(
    0xeb24,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb3d: IconData(
    0xeb3a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe95d: IconData(
    0xe95c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe239: IconData(
    0xe238,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed51: IconData(
    0xed50,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe235: IconData(
    0xe234,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe703: IconData(
    0xe702,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe237: IconData(
    0xe236,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb19: IconData(
    0xeb18,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb21: IconData(
    0xeb20,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb2d: IconData(
    0xeb2c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb29: IconData(
    0xeb28,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed4f: IconData(
    0xed4e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed09: IconData(
    0xed08,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe23b: IconData(
    0xe23a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb27: IconData(
    0xeb26,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb3f: IconData(
    0xeb3c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb43: IconData(
    0xeb35,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea23: IconData(
    0xea22,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb47: IconData(
    0xeb3e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe23d: IconData(
    0xe23c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb23: IconData(
    0xeb22,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe959: IconData(
    0xe958,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe711: IconData(
    0xe710,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8c1: IconData(
    0xe8c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb51: IconData(
    0xeb50,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8c3: IconData(
    0xe8c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe793: IconData(
    0xe792,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe23f: IconData(
    0xe23e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe241: IconData(
    0xe240,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe56d: IconData(
    0xe56c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe243: IconData(
    0xe242,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9e9: IconData(
    0xe9e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe621: IconData(
    0xe620,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe575: IconData(
    0xe574,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe56f: IconData(
    0xe56e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe571: IconData(
    0xe570,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe729: IconData(
    0xe728,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe72b: IconData(
    0xe72a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe245: IconData(
    0xe244,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe623: IconData(
    0xe622,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecf3: IconData(
    0xecf2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea39: IconData(
    0xea38,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecf1: IconData(
    0xecf0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe625: IconData(
    0xe624,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe247: IconData(
    0xe246,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe79f: IconData(
    0xe79e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed6b: IconData(
    0xed6a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed6d: IconData(
    0xed6c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe249: IconData(
    0xe248,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaf5: IconData(
    0xeaf4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6ed: IconData(
    0xe6ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe75f: IconData(
    0xe75e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6cd: IconData(
    0xe6cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeacd: IconData(
    0xeacc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb4b: IconData(
    0xeb4a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe24b: IconData(
    0xe24a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8f9: IconData(
    0xe8f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea3d: IconData(
    0xea3c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe255: IconData(
    0xe254,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe257: IconData(
    0xe256,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe259: IconData(
    0xe258,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe25b: IconData(
    0xe25a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec2b: IconData(
    0xec2a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb5f: IconData(
    0xeb5e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe25d: IconData(
    0xe25c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe25f: IconData(
    0xe25e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec2f: IconData(
    0xec2e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb61: IconData(
    0xeb60,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea87: IconData(
    0xea86,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb4c: IconData(
    0xeb46,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe261: IconData(
    0xe260,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe719: IconData(
    0xe718,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee4d: IconData(
    0xee4c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea89: IconData(
    0xea88,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe263: IconData(
    0xe262,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea5d: IconData(
    0xea5c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe627: IconData(
    0xe626,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe265: IconData(
    0xe264,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebe5: IconData(
    0xebe4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe267: IconData(
    0xe266,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe269: IconData(
    0xe268,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe26b: IconData(
    0xe26a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe26d: IconData(
    0xe26c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe26f: IconData(
    0xe26e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecd7: IconData(
    0xecd6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8cf: IconData(
    0xe8ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe769: IconData(
    0xe768,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe629: IconData(
    0xe628,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea33: IconData(
    0xea32,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe271: IconData(
    0xe270,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe87d: IconData(
    0xe87c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe273: IconData(
    0xe272,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6e1: IconData(
    0xe6e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6e7: IconData(
    0xe6e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6e3: IconData(
    0xe6e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6eb: IconData(
    0xe6ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6e5: IconData(
    0xe6e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6e9: IconData(
    0xe6e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe62b: IconData(
    0xe62a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe275: IconData(
    0xe274,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe277: IconData(
    0xe276,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe279: IconData(
    0xe278,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe27b: IconData(
    0xe27a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe27d: IconData(
    0xe27c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe27f: IconData(
    0xe27e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe281: IconData(
    0xe280,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe283: IconData(
    0xe282,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe577: IconData(
    0xe576,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe695: IconData(
    0xe694,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe697: IconData(
    0xe696,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe289: IconData(
    0xe288,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe28b: IconData(
    0xe28a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe28d: IconData(
    0xe28c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe28f: IconData(
    0xe28e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe285: IconData(
    0xe284,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe291: IconData(
    0xe290,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe287: IconData(
    0xe286,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecb5: IconData(
    0xecb4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea3f: IconData(
    0xea3e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed11: IconData(
    0xed10,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7b7: IconData(
    0xe7b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe977: IconData(
    0xe976,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8f7: IconData(
    0xe8f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe293: IconData(
    0xe292,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb93: IconData(
    0xeb92,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe295: IconData(
    0xe294,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb95: IconData(
    0xeb94,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedd9: IconData(
    0xedd8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedd7: IconData(
    0xedd6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedd5: IconData(
    0xedd4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb4d: IconData(
    0xeb42,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe62d: IconData(
    0xe62c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec69: IconData(
    0xec68,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec6b: IconData(
    0xec6a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb59: IconData(
    0xeb58,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe613: IconData(
    0xe612,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedc5: IconData(
    0xedc4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeda3: IconData(
    0xeda2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe297: IconData(
    0xe296,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec8d: IconData(
    0xec8c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea8b: IconData(
    0xea8a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea67: IconData(
    0xea66,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe791: IconData(
    0xe790,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe80f: IconData(
    0xe80e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe299: IconData(
    0xe298,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea4f: IconData(
    0xea4e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee5b: IconData(
    0xee5a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea8d: IconData(
    0xea8c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee83: IconData(
    0xee82,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea4d: IconData(
    0xea4c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe57b: IconData(
    0xe57a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe57d: IconData(
    0xe57c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe811: IconData(
    0xe810,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe57f: IconData(
    0xe57e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7cd: IconData(
    0xe7cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe29b: IconData(
    0xe29a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe631: IconData(
    0xe630,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec95: IconData(
    0xec94,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec93: IconData(
    0xec92,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec91: IconData(
    0xec90,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe581: IconData(
    0xe580,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee81: IconData(
    0xee80,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe29d: IconData(
    0xe29c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe62f: IconData(
    0xe62e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6a1: IconData(
    0xe6a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecc9: IconData(
    0xecc8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe583: IconData(
    0xe582,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe29f: IconData(
    0xe29e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2a1: IconData(
    0xe2a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed47: IconData(
    0xed46,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2a3: IconData(
    0xe2a2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2a5: IconData(
    0xe2a4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7d5: IconData(
    0xe7d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6ff: IconData(
    0xe6fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2a7: IconData(
    0xe2a6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe585: IconData(
    0xe584,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2a9: IconData(
    0xe2a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebe9: IconData(
    0xebe8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec49: IconData(
    0xec48,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2ab: IconData(
    0xe2aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb99: IconData(
    0xeb98,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2ad: IconData(
    0xe2ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2af: IconData(
    0xe2ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea8f: IconData(
    0xea8e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8e9: IconData(
    0xe8e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec77: IconData(
    0xec76,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe633: IconData(
    0xe632,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec87: IconData(
    0xec86,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecd1: IconData(
    0xecd0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2b1: IconData(
    0xe2b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe845: IconData(
    0xe844,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2b3: IconData(
    0xe2b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2b5: IconData(
    0xe2b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2b7: IconData(
    0xe2b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2b9: IconData(
    0xe2b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2bb: IconData(
    0xe2ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2bd: IconData(
    0xe2bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2bf: IconData(
    0xe2be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2c1: IconData(
    0xe2c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2c3: IconData(
    0xe2c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2c5: IconData(
    0xe2c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2c7: IconData(
    0xe2c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe88f: IconData(
    0xe88e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe805: IconData(
    0xe804,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6f7: IconData(
    0xe6f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2c9: IconData(
    0xe2c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2cb: IconData(
    0xe2ca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7a9: IconData(
    0xe7a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2cd: IconData(
    0xe2cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe837: IconData(
    0xe836,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe835: IconData(
    0xe834,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe635: IconData(
    0xe634,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2cf: IconData(
    0xe2ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2d1: IconData(
    0xe2d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2d3: IconData(
    0xe2d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe87b: IconData(
    0xe87a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecc5: IconData(
    0xecc4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedbb: IconData(
    0xedba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee43: IconData(
    0xee42,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee07: IconData(
    0xee06,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7e3: IconData(
    0xe7e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7e5: IconData(
    0xe7e1,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2d5: IconData(
    0xe2d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea5f: IconData(
    0xea5e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb55: IconData(
    0xeb54,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2d7: IconData(
    0xe2d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe783: IconData(
    0xe782,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2d9: IconData(
    0xe2d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea79: IconData(
    0xea78,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe637: IconData(
    0xe636,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9e5: IconData(
    0xe9e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec27: IconData(
    0xec26,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe639: IconData(
    0xe638,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee2f: IconData(
    0xee2e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe587: IconData(
    0xe586,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedc7: IconData(
    0xedc6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe843: IconData(
    0xe842,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6d7: IconData(
    0xe6d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2db: IconData(
    0xe2da,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe95b: IconData(
    0xe95a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8c8: IconData(
    0xe8c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8c9: IconData(
    0xe8c7,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedad: IconData(
    0xedac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeda5: IconData(
    0xeda4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebf9: IconData(
    0xebf8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec09: IconData(
    0xec08,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec15: IconData(
    0xec14,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe63b: IconData(
    0xe63a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2dd: IconData(
    0xe2dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe63d: IconData(
    0xe63c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9f7: IconData(
    0xe9f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2df: IconData(
    0xe2de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea85: IconData(
    0xea84,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2e1: IconData(
    0xe2e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6d3: IconData(
    0xe6d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6d5: IconData(
    0xe6d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed71: IconData(
    0xed70,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2e3: IconData(
    0xe2e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2e5: IconData(
    0xe2e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2e7: IconData(
    0xe2e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2e9: IconData(
    0xe2e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2eb: IconData(
    0xe2ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2ed: IconData(
    0xe2ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2ef: IconData(
    0xe2ee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedef: IconData(
    0xedee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb03: IconData(
    0xeb02,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2f1: IconData(
    0xe2f0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2f3: IconData(
    0xe2f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeadd: IconData(
    0xeadc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2f5: IconData(
    0xe2f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebdf: IconData(
    0xebde,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebe1: IconData(
    0xebe0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2f7: IconData(
    0xe2f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2f9: IconData(
    0xe2f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebdd: IconData(
    0xebdc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2fb: IconData(
    0xe2fa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe2ff: IconData(
    0xe2fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe301: IconData(
    0xe300,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe303: IconData(
    0xe302,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe305: IconData(
    0xe304,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe307: IconData(
    0xe306,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe309: IconData(
    0xe308,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe30b: IconData(
    0xe30a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecb9: IconData(
    0xecb8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed83: IconData(
    0xed82,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6b7: IconData(
    0xe6b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe681: IconData(
    0xe680,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe683: IconData(
    0xe682,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe30d: IconData(
    0xe30c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe30f: IconData(
    0xe30e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe311: IconData(
    0xe310,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec1f: IconData(
    0xec1e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe317: IconData(
    0xe316,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee3b: IconData(
    0xee3a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe319: IconData(
    0xe318,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe315: IconData(
    0xe314,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee3f: IconData(
    0xee3e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee3d: IconData(
    0xee3c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee39: IconData(
    0xee38,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe31b: IconData(
    0xe31a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe509: IconData(
    0xe508,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe641: IconData(
    0xe640,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe31d: IconData(
    0xe31c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9f5: IconData(
    0xe9f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb9f: IconData(
    0xeb9e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed69: IconData(
    0xed68,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe31f: IconData(
    0xe31e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed65: IconData(
    0xed64,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe321: IconData(
    0xe320,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecfd: IconData(
    0xecfc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe323: IconData(
    0xe322,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe325: IconData(
    0xe324,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe643: IconData(
    0xe642,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedc3: IconData(
    0xedc2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9c5: IconData(
    0xe9c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6d9: IconData(
    0xe6d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed03: IconData(
    0xed02,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9bb: IconData(
    0xe9ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec8f: IconData(
    0xec8e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe327: IconData(
    0xe326,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe329: IconData(
    0xe328,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe75d: IconData(
    0xe75c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec7b: IconData(
    0xec7a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb6d: IconData(
    0xeb6c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb71: IconData(
    0xeb70,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeacf: IconData(
    0xeace,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb67: IconData(
    0xeb66,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb6b: IconData(
    0xeb6a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe32b: IconData(
    0xe32a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe32d: IconData(
    0xe32c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed53: IconData(
    0xed4c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe589: IconData(
    0xe588,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee69: IconData(
    0xee68,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe32f: IconData(
    0xe32e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe58b: IconData(
    0xe58a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe58d: IconData(
    0xe58c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe331: IconData(
    0xe330,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe58f: IconData(
    0xe58e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe825: IconData(
    0xe824,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe823: IconData(
    0xe822,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecef: IconData(
    0xecee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe80b: IconData(
    0xe80a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7af: IconData(
    0xe7ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe33b: IconData(
    0xe33a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe335: IconData(
    0xe334,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe339: IconData(
    0xe338,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe337: IconData(
    0xe336,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe333: IconData(
    0xe332,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe645: IconData(
    0xe644,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe33d: IconData(
    0xe33c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe33f: IconData(
    0xe33e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe341: IconData(
    0xe340,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee0d: IconData(
    0xee0c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb7d: IconData(
    0xeb7c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe343: IconData(
    0xe342,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeadf: IconData(
    0xeade,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe82f: IconData(
    0xe82e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeddf: IconData(
    0xedde,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeddd: IconData(
    0xeddc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeddb: IconData(
    0xedda,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe345: IconData(
    0xe344,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe347: IconData(
    0xe346,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeda7: IconData(
    0xeda6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedaf: IconData(
    0xedae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedb1: IconData(
    0xedb0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedb3: IconData(
    0xedb2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed3b: IconData(
    0xed3a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe349: IconData(
    0xe348,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe34b: IconData(
    0xe34a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe34d: IconData(
    0xe34c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe34f: IconData(
    0xe34e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe63f: IconData(
    0xe63e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6fb: IconData(
    0xe6fa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9a1: IconData(
    0xe9a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed7d: IconData(
    0xed7c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe353: IconData(
    0xe352,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe359: IconData(
    0xe358,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe35f: IconData(
    0xe35e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe365: IconData(
    0xe364,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe36b: IconData(
    0xe36a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe371: IconData(
    0xe370,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe377: IconData(
    0xe376,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe37d: IconData(
    0xe37c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe383: IconData(
    0xe382,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe389: IconData(
    0xe388,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe351: IconData(
    0xe350,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe357: IconData(
    0xe356,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe35d: IconData(
    0xe35c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe363: IconData(
    0xe362,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe369: IconData(
    0xe368,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe36f: IconData(
    0xe36e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe375: IconData(
    0xe374,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe355: IconData(
    0xe354,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe35b: IconData(
    0xe35a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe361: IconData(
    0xe360,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe367: IconData(
    0xe366,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe36d: IconData(
    0xe36c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe373: IconData(
    0xe372,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe379: IconData(
    0xe378,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe37f: IconData(
    0xe37e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe385: IconData(
    0xe384,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe38b: IconData(
    0xe38a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe37b: IconData(
    0xe37a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe381: IconData(
    0xe380,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe387: IconData(
    0xe386,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3c9: IconData(
    0xe3c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe38d: IconData(
    0xe38c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe647: IconData(
    0xe646,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe38f: IconData(
    0xe38e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea47: IconData(
    0xea46,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee2d: IconData(
    0xee2c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7d3: IconData(
    0xe7d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8a9: IconData(
    0xe8a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee41: IconData(
    0xee40,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed37: IconData(
    0xed36,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed8d: IconData(
    0xed8c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe391: IconData(
    0xe390,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6f1: IconData(
    0xe6f0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe591: IconData(
    0xe590,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6f3: IconData(
    0xe6f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe393: IconData(
    0xe392,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6f5: IconData(
    0xe6f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6c9: IconData(
    0xe6c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaa3: IconData(
    0xeaa2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec89: IconData(
    0xec88,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe395: IconData(
    0xe394,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe397: IconData(
    0xe396,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe399: IconData(
    0xe398,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe39b: IconData(
    0xe39a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe593: IconData(
    0xe592,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea7d: IconData(
    0xea7c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe961: IconData(
    0xe960,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecc7: IconData(
    0xecc6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecb3: IconData(
    0xecb2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe753: IconData(
    0xe752,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe39d: IconData(
    0xe39c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe98b: IconData(
    0xe98a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe39f: IconData(
    0xe39e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3a1: IconData(
    0xe3a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe649: IconData(
    0xe648,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe98d: IconData(
    0xe98c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3a3: IconData(
    0xe3a2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3ab: IconData(
    0xe3aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3ad: IconData(
    0xe3ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe64b: IconData(
    0xe64a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3af: IconData(
    0xe3ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3b1: IconData(
    0xe3b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3b3: IconData(
    0xe3b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe907: IconData(
    0xe906,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3b5: IconData(
    0xe3b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebc7: IconData(
    0xebc6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecf7: IconData(
    0xecf6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecf9: IconData(
    0xecf8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec7f: IconData(
    0xec7e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec5d: IconData(
    0xec5c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe94b: IconData(
    0xe94a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3b7: IconData(
    0xe3b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3a9: IconData(
    0xe3a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecff: IconData(
    0xecfe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe72f: IconData(
    0xe72e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe735: IconData(
    0xe734,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee59: IconData(
    0xee58,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed55: IconData(
    0xed54,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe731: IconData(
    0xe730,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe71d: IconData(
    0xe71c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe71f: IconData(
    0xe71e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe737: IconData(
    0xe736,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed5d: IconData(
    0xed5c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe733: IconData(
    0xe732,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe73b: IconData(
    0xe73a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebe7: IconData(
    0xebe6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3b9: IconData(
    0xe3b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3bb: IconData(
    0xe3ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3bd: IconData(
    0xe3bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3bf: IconData(
    0xe3be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3cd: IconData(
    0xe3cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3c1: IconData(
    0xe3c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3cb: IconData(
    0xe3ca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec57: IconData(
    0xec56,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3c3: IconData(
    0xe3c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3c7: IconData(
    0xe3c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3c5: IconData(
    0xe3c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3cf: IconData(
    0xe3ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec81: IconData(
    0xec80,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9c9: IconData(
    0xe9c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee27: IconData(
    0xee26,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe64d: IconData(
    0xe64c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea05: IconData(
    0xea04,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe701: IconData(
    0xe700,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea43: IconData(
    0xea42,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedd1: IconData(
    0xedd0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe64f: IconData(
    0xe64e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb9d: IconData(
    0xeb9c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed87: IconData(
    0xed86,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed89: IconData(
    0xed88,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecc3: IconData(
    0xecc2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe797: IconData(
    0xe796,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe651: IconData(
    0xe650,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe653: IconData(
    0xe652,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebaf: IconData(
    0xebae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3d1: IconData(
    0xe3d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3d3: IconData(
    0xe3d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8bf: IconData(
    0xe8be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6ab: IconData(
    0xe6aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe947: IconData(
    0xe946,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb5d: IconData(
    0xeb5c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb57: IconData(
    0xeb56,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb5b: IconData(
    0xeb5a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3d5: IconData(
    0xe3d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3d7: IconData(
    0xe3d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3d9: IconData(
    0xe3d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed56: IconData(
    0xed4a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe595: IconData(
    0xe594,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec4b: IconData(
    0xec4a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6d1: IconData(
    0xe6d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb4f: IconData(
    0xeb4e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebbf: IconData(
    0xebbe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec23: IconData(
    0xec22,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3db: IconData(
    0xe3da,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7a3: IconData(
    0xe7a2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe655: IconData(
    0xe654,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe657: IconData(
    0xe656,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3dd: IconData(
    0xe3dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3df: IconData(
    0xe3de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3e1: IconData(
    0xe3e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe659: IconData(
    0xe658,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe65b: IconData(
    0xe65a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe001: IconData(
    0xe000,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3e3: IconData(
    0xe3e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe65d: IconData(
    0xe65c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe65f: IconData(
    0xe65e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3e5: IconData(
    0xe3e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe597: IconData(
    0xe596,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3e7: IconData(
    0xe3e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3eb: IconData(
    0xe3e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3ed: IconData(
    0xe3e9,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6ad: IconData(
    0xe6ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe661: IconData(
    0xe660,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeac3: IconData(
    0xeac2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee03: IconData(
    0xee02,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3ef: IconData(
    0xe3ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe77f: IconData(
    0xe77e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb09: IconData(
    0xeb08,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9dd: IconData(
    0xe9dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe599: IconData(
    0xe598,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe59b: IconData(
    0xe59a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed63: IconData(
    0xed62,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed0d: IconData(
    0xed0c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3f1: IconData(
    0xe3ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed41: IconData(
    0xed40,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3f3: IconData(
    0xe3ee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3f5: IconData(
    0xe3f0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3f7: IconData(
    0xe3f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe75b: IconData(
    0xe75a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe59d: IconData(
    0xe59c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3f9: IconData(
    0xe3f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3fb: IconData(
    0xe3f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb8b: IconData(
    0xeb8a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed6f: IconData(
    0xed6e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6a9: IconData(
    0xe6a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3fd: IconData(
    0xe3fa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe839: IconData(
    0xe838,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe763: IconData(
    0xe762,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe3ff: IconData(
    0xe3fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe401: IconData(
    0xe3fe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5a3: IconData(
    0xe5a2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe59f: IconData(
    0xe59e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5a1: IconData(
    0xe5a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe403: IconData(
    0xe400,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe405: IconData(
    0xe402,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea1b: IconData(
    0xea1a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6b9: IconData(
    0xe6b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe78b: IconData(
    0xe78a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe751: IconData(
    0xe750,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebb7: IconData(
    0xebb6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebb5: IconData(
    0xebb4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeae1: IconData(
    0xeae0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe821: IconData(
    0xe820,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe407: IconData(
    0xe404,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe86f: IconData(
    0xe86e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe807: IconData(
    0xe806,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe663: IconData(
    0xe662,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb7b: IconData(
    0xeb7a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe605: IconData(
    0xe604,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe607: IconData(
    0xe606,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe60b: IconData(
    0xe60a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe609: IconData(
    0xe608,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe60d: IconData(
    0xe60c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb8f: IconData(
    0xeb8e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedff: IconData(
    0xedfe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeca5: IconData(
    0xeca4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe69b: IconData(
    0xe69a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe747: IconData(
    0xe746,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaf9: IconData(
    0xeaf8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaf7: IconData(
    0xeaf6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe745: IconData(
    0xe744,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe69d: IconData(
    0xe69c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe69f: IconData(
    0xe69e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec5f: IconData(
    0xec5e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe409: IconData(
    0xe406,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed57: IconData(
    0xed52,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe40b: IconData(
    0xe408,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe40d: IconData(
    0xe40a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe40f: IconData(
    0xe40c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe709: IconData(
    0xe708,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe411: IconData(
    0xe40e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe707: IconData(
    0xe706,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe413: IconData(
    0xe410,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec35: IconData(
    0xec34,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe414: IconData(
    0xe412,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe78d: IconData(
    0xe78c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea93: IconData(
    0xea92,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecfb: IconData(
    0xecfa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe417: IconData(
    0xe416,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe419: IconData(
    0xe418,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe41f: IconData(
    0xe41e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe421: IconData(
    0xe420,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9e7: IconData(
    0xe9e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe777: IconData(
    0xe776,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeab5: IconData(
    0xeab4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe423: IconData(
    0xe422,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe425: IconData(
    0xe424,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe427: IconData(
    0xe426,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeab7: IconData(
    0xeab6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec25: IconData(
    0xec24,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeab9: IconData(
    0xeab8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe429: IconData(
    0xe428,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe42b: IconData(
    0xe42a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebad: IconData(
    0xebac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe89d: IconData(
    0xe89c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe665: IconData(
    0xe664,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9b9: IconData(
    0xe9b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe42d: IconData(
    0xe42c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5a5: IconData(
    0xe5a4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe42f: IconData(
    0xe42e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5a7: IconData(
    0xe5a6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe431: IconData(
    0xe430,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe917: IconData(
    0xe916,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8dd: IconData(
    0xe8dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5a9: IconData(
    0xe5a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe433: IconData(
    0xe432,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe435: IconData(
    0xe434,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed33: IconData(
    0xed32,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe437: IconData(
    0xe436,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec63: IconData(
    0xec62,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe439: IconData(
    0xe438,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe43b: IconData(
    0xe43a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee57: IconData(
    0xee56,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe43d: IconData(
    0xe43c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe43f: IconData(
    0xe43e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe441: IconData(
    0xe440,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe667: IconData(
    0xe666,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe443: IconData(
    0xe442,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe669: IconData(
    0xe668,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe80d: IconData(
    0xe80c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed61: IconData(
    0xed60,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5ab: IconData(
    0xe5aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe717: IconData(
    0xe716,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeccf: IconData(
    0xecce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed7e: IconData(
    0xed7a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed7f: IconData(
    0xed7b,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe445: IconData(
    0xe444,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe447: IconData(
    0xe446,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8df: IconData(
    0xe8de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe449: IconData(
    0xe448,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6a3: IconData(
    0xe6a2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea09: IconData(
    0xea08,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe44b: IconData(
    0xe44a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe44d: IconData(
    0xe44c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe44f: IconData(
    0xe44e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe451: IconData(
    0xe450,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe453: IconData(
    0xe452,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe455: IconData(
    0xe454,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe457: IconData(
    0xe456,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe459: IconData(
    0xe458,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe45b: IconData(
    0xe45a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe45d: IconData(
    0xe45c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee75: IconData(
    0xee74,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee67: IconData(
    0xee66,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe66b: IconData(
    0xe66a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee29: IconData(
    0xee28,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe66d: IconData(
    0xe66c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9fb: IconData(
    0xe9fa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe873: IconData(
    0xe872,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe877: IconData(
    0xe876,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe66f: IconData(
    0xe66e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7e8: IconData(
    0xe7e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe45f: IconData(
    0xe45e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe463: IconData(
    0xe462,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb17: IconData(
    0xeb16,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe691: IconData(
    0xe690,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe871: IconData(
    0xe870,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe875: IconData(
    0xe874,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe465: IconData(
    0xe464,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe467: IconData(
    0xe466,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedf5: IconData(
    0xedf4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb79: IconData(
    0xeb78,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedf7: IconData(
    0xedf6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe469: IconData(
    0xe468,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8ed: IconData(
    0xe8ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea49: IconData(
    0xea48,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea91: IconData(
    0xea90,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe46b: IconData(
    0xe46a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecf5: IconData(
    0xecf4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6a5: IconData(
    0xe6a4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe70b: IconData(
    0xe70a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe89f: IconData(
    0xe89e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xead5: IconData(
    0xead4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9ad: IconData(
    0xe9ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecbf: IconData(
    0xecbe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7eb: IconData(
    0xe7ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5ad: IconData(
    0xe5ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea45: IconData(
    0xea44,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe46d: IconData(
    0xe46c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe46f: IconData(
    0xe46e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe471: IconData(
    0xe470,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea3b: IconData(
    0xea3a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe699: IconData(
    0xe698,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe73f: IconData(
    0xe73e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedc1: IconData(
    0xedc0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedb7: IconData(
    0xedb6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1a9: IconData(
    0xe1a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe1a7: IconData(
    0xe1a6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebd7: IconData(
    0xebd6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xebd5: IconData(
    0xebd4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe499: IconData(
    0xe498,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5af: IconData(
    0xe5ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9b1: IconData(
    0xe9b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5b1: IconData(
    0xe5b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe473: IconData(
    0xe472,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe475: IconData(
    0xe474,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5b7: IconData(
    0xe5b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe817: IconData(
    0xe816,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedb9: IconData(
    0xedb8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedb5: IconData(
    0xedb4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe83d: IconData(
    0xe83c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5b9: IconData(
    0xe5b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecb7: IconData(
    0xecb6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5bb: IconData(
    0xe5ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeced: IconData(
    0xecec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe969: IconData(
    0xe968,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe671: IconData(
    0xe670,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe477: IconData(
    0xe476,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe779: IconData(
    0xe778,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe479: IconData(
    0xe478,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe673: IconData(
    0xe672,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe47b: IconData(
    0xe47a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe47d: IconData(
    0xe47c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe903: IconData(
    0xe902,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8e7: IconData(
    0xe8e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5bd: IconData(
    0xe5bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe755: IconData(
    0xe754,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeae7: IconData(
    0xeae6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe721: IconData(
    0xe720,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8bb: IconData(
    0xe8ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe47f: IconData(
    0xe47e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeae9: IconData(
    0xeae8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7a1: IconData(
    0xe7a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed35: IconData(
    0xed34,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6ef: IconData(
    0xe6ee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe481: IconData(
    0xe480,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe483: IconData(
    0xe482,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe485: IconData(
    0xe484,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe487: IconData(
    0xe486,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5bf: IconData(
    0xe5be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec97: IconData(
    0xec96,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6bb: IconData(
    0xe6ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6c5: IconData(
    0xe6c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6c3: IconData(
    0xe6c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6bd: IconData(
    0xe6bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6c7: IconData(
    0xe6c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6c1: IconData(
    0xe6c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6bf: IconData(
    0xe6be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea1f: IconData(
    0xea1e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5c1: IconData(
    0xe5c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea1d: IconData(
    0xea1c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5c3: IconData(
    0xe5c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec99: IconData(
    0xec98,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec9b: IconData(
    0xec9a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe48b: IconData(
    0xe48a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe489: IconData(
    0xe488,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5c5: IconData(
    0xe5c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeb0b: IconData(
    0xeb0a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5c7: IconData(
    0xe5c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5c9: IconData(
    0xe5c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5cb: IconData(
    0xe5ca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5cd: IconData(
    0xe5cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed9f: IconData(
    0xed9e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea5b: IconData(
    0xea5a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe48d: IconData(
    0xe48c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe48f: IconData(
    0xe48e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe491: IconData(
    0xe490,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed1d: IconData(
    0xed1c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaf3: IconData(
    0xeaf2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeda9: IconData(
    0xeda8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe493: IconData(
    0xe492,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7e9: IconData(
    0xe7e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed31: IconData(
    0xed30,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedd3: IconData(
    0xedd2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe675: IconData(
    0xe674,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe677: IconData(
    0xe676,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe79b: IconData(
    0xe79a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe79d: IconData(
    0xe79c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeca1: IconData(
    0xeca0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9cd: IconData(
    0xe9cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe88d: IconData(
    0xe88c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe495: IconData(
    0xe494,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe679: IconData(
    0xe678,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xede7: IconData(
    0xede6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec6f: IconData(
    0xec6e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9f1: IconData(
    0xe9f0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe415: IconData(
    0xe3f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9a9: IconData(
    0xe9a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe67b: IconData(
    0xe67a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9ab: IconData(
    0xe9aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe497: IconData(
    0xe496,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe49f: IconData(
    0xe49e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4a1: IconData(
    0xe4a0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9ed: IconData(
    0xe9ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4a3: IconData(
    0xe4a2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4a7: IconData(
    0xe4a6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4a9: IconData(
    0xe4a8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4ab: IconData(
    0xe4aa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe011: IconData(
    0xe010,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee53: IconData(
    0xee52,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xede3: IconData(
    0xede2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6db: IconData(
    0xe6da,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6dd: IconData(
    0xe6dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe91b: IconData(
    0xe91a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe67d: IconData(
    0xe67c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee49: IconData(
    0xee48,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4ad: IconData(
    0xe4ac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4af: IconData(
    0xe4ae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4b1: IconData(
    0xe4b0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4b3: IconData(
    0xe4b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5b3: IconData(
    0xe5b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5b5: IconData(
    0xe5b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe67f: IconData(
    0xe67e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4b5: IconData(
    0xe4b4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4b7: IconData(
    0xe4b6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8d5: IconData(
    0xe8d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5cf: IconData(
    0xe5ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4bb: IconData(
    0xe4ba,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe685: IconData(
    0xe684,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe687: IconData(
    0xe686,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedbf: IconData(
    0xedbe,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe87f: IconData(
    0xe87e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe879: IconData(
    0xe878,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4bf: IconData(
    0xe4be,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4c1: IconData(
    0xe4c0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe957: IconData(
    0xe956,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4c3: IconData(
    0xe4c2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeafb: IconData(
    0xeafa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4c5: IconData(
    0xe4c4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec39: IconData(
    0xec38,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xec37: IconData(
    0xec36,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4c7: IconData(
    0xe4c6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4c9: IconData(
    0xe4c8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4cb: IconData(
    0xe4ca,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6fd: IconData(
    0xe6fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4cd: IconData(
    0xe4cc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe73d: IconData(
    0xe73c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4cf: IconData(
    0xe4ce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4d1: IconData(
    0xe4d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4d3: IconData(
    0xe4d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeca9: IconData(
    0xeca8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4d5: IconData(
    0xe4d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe757: IconData(
    0xe756,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4d7: IconData(
    0xe4d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe68d: IconData(
    0xe68c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe68f: IconData(
    0xe68e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe827: IconData(
    0xe826,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe76f: IconData(
    0xe76e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee63: IconData(
    0xee62,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee65: IconData(
    0xee64,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4d9: IconData(
    0xe4d8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe741: IconData(
    0xe740,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4db: IconData(
    0xe4da,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4dd: IconData(
    0xe4dc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xedcf: IconData(
    0xedce,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeba3: IconData(
    0xeba2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecad: IconData(
    0xecac,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7b9: IconData(
    0xe7b8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7d7: IconData(
    0xe7d6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xee2b: IconData(
    0xee2a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4df: IconData(
    0xe4de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe727: IconData(
    0xe726,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe689: IconData(
    0xe688,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe68b: IconData(
    0xe68a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecd5: IconData(
    0xecd4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4e1: IconData(
    0xe4e0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4e3: IconData(
    0xe4e2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe7fd: IconData(
    0xe7fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4e5: IconData(
    0xe4e4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xede9: IconData(
    0xede8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4e7: IconData(
    0xe4e6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea9d: IconData(
    0xea9c,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea9b: IconData(
    0xea9a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xea9f: IconData(
    0xea9e,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xeaa1: IconData(
    0xeaa0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe803: IconData(
    0xe802,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe801: IconData(
    0xe800,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6df: IconData(
    0xe6de,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9b3: IconData(
    0xe9b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecdd: IconData(
    0xecdc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xecaf: IconData(
    0xecae,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe8d3: IconData(
    0xe8d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5d1: IconData(
    0xe5d0,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4e9: IconData(
    0xe4e8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe89b: IconData(
    0xe89a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4eb: IconData(
    0xe4ea,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4ed: IconData(
    0xe4ec,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4ef: IconData(
    0xe4ee,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4f3: IconData(
    0xe4f2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4f5: IconData(
    0xe4f4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5d3: IconData(
    0xe5d2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe9f9: IconData(
    0xe9f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe693: IconData(
    0xe692,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe6b3: IconData(
    0xe6b2,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe5d5: IconData(
    0xe5d4,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4f7: IconData(
    0xe4f6,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4f9: IconData(
    0xe4f8,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4bd: IconData(
    0xe4bc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4fb: IconData(
    0xe4fa,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xed9b: IconData(
    0xed9a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe92b: IconData(
    0xe92a,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
  0xe4fd: IconData(
    0xe4fc,
    fontFamily: 'PhosphorDuotone',
    fontPackage: 'phosphor_flutter',
    matchTextDirection: true,
  ),
};
